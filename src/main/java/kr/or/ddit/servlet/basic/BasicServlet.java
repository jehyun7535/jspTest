...
public class BasicServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//  로딩시 최초 1회	사용자가 요청을 할 때마다    서버 종료 or reload
		
		// init 메소드는 해당 서블릿에서 사용할 자원을 초기화 할 때 사용
		// 로딩시 최초1회 호출 : 로딩 되는 시점 ==> 해당서블릿으로 최초 요청이 들어왔을 때 
		// 단 web.xml의 servlet 엘레멘트의 하위 엘레멘트인 loard-on-startup 엘레멘트 값으로
		// 양의 정수값을 입력할 경우 서버가 가동하면서 바로 init 메소드를 호출한다
		...
		pw.flush();		//더이상 작성할 내용이 없으므로 작업을 마무리 한단
		pw.close();		//사용할 자원 반납
	}
